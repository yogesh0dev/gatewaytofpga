	<?php 
		...
		echo form_open_multipart('stcourses/edit/'.$stcourses['id'],$attributes); 
	?>
	<?php if(@$error['error']){?>
...
<?php }?>
	<div>
		<span class="text-danger">*</span><label class="form-label">stcourses : </label>
		<input class="form-control"placeholder='stcourses' type="text" name='stcourses' value="<?php echo ($this->input->post('status') ? $this->input->post('status') : $stcourses['stcourses']); ?>" />
		<span class="text-danger"><?php echo form_error('stcourses');?></span>
	</div>
