	<?php 
		...
		echo form_open_multipart('attendance/edit/'.$attendance['id'],$attributes); 
	?>
	...
	<div>
		...
		<select  name='course_id' class="form-select" id="select1" aria-label="Example select with button addon">
			...
			{
				$selected = ($c['id'] == $attendance['course_id']) ? ' selected="selected"' : "";
	...
	<div>
		...
		<select  name="studentid" class="form-select" id="select2" aria-label="Example select with button addon">
			...
			{
				if($c['role']=='student'){
					$selected = ($c['id'] == $attendance['studentid']) ? ' selected="selected"' : "";
	...
	<div>
		...
		<select  name="trainerid" class="form-select" id="select3" aria-label="Example select with button addon">
			...
			{
				if($c['role']=='teacher'){
					$selected = ($c['id'] == $attendance['trainerid']) ? ' selected="selected"' : "";
					echo '<option '.$selected.' value="'.$c['id'].'">'.$c['first_name'].''.$c['last_name'].'</option>';
				}
			} 
			?>
	...
	<div>
		...
		<select  name='batch' class="form-select" id="select4" aria-label="Example select with button addon">
			...
			{
				$selected = ($c['id'] == $attendance['batch']) ? ' selected="selected"' : "";
	...
	<div>
		<span class="text-danger">*</span><label class="form-label">Attendence Date : </label>
		<input class="form-control"placeholder='Adate' type="date" name='adate' value="<?php echo ($this->input->post('status') ? $this->input->post('status') : $attendance['adate']); ?>" />
